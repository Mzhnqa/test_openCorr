...
int main()
{
	...
	string ref_image_path = "E:/openCorr_sift_test/test/rotation_000.tif";
	string tar_image_path = "E:/openCorr_sift_test/test/rotation_170.tif";
	...
	//save the coordinates of matched keypoints 保存匹配关键点的坐标
	...
	//FeatureAffine instance estimates the deformation at POI according the neighbor features   FeatureAffine实例根据邻近特征点来估计兴趣点处的变形。
	...
	file_path = tar_image_path.substr(0, tar_image_path.find_last_of(".")) + "_sift_FeatureAffine2D_r12.csv";
	...
	//ICGN with the 2nd order shape fucntion  计算了poi点
	...
	//ICGN with the 2nd order shape fucntion  计算了sift匹配的关键点
	ICGN2D2* icgn2 = new ICGN2D2(subset_radius_x, subset_radius_y, max_deformation_norm, max_iteration, cpu_thread_number);
	icgn2->setImages(ref_img, tar_img);
	icgn2->prepare();
	vector<Point2D> matched_queue;
	matched_queue = sift->ref_matched_kp;
	icgn2->compute(matched_queue);
